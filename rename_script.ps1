...
$srcDir = "ASSETS\kashish ad® - Google image"
$destDir = "public\assets\images"

if (-not (Test-Path $destDir)) { New-Item -ItemType Directory -Path $destDir -Force }
Get-ChildItem -Path $destDir | Remove-Item -Force

foreach ($old in $map.Keys) {
    $new = $map[$old]
    $oldPath = Join-Path $srcDir $old
    $newDest = Join-Path $destDir $new

    if (Test-Path $oldPath) {
        Copy-Item -Path $oldPath -Destination $newDest -Force
        Rename-Item -Path $oldPath -NewName $new -Force -ErrorAction SilentlyContinue
    }
}

Write-Output "Done. Public assets image count:"
(Get-ChildItem $destDir).Count
Write-Output "Source assets image count:"
(Get-ChildItem $srcDir).Count
